...
const AddNewArrival = () => {
  ...
  const [modifiedPerfumes, setModifiedPerfumes] = useState([])
  ...
  useEffect(() => {
    if (perfumes.length > 0) {
      const temp = perfumes.map(item => {
        return { value: item?._id, label: item?.perfume, banner: item?.banner }
      })
      setModifiedPerfumes(temp)
  ...
  const onSubmit = (data) => {
    ...
    axios
      ...
      .post(`${import.meta.env.VITE_API_URL}/newArrival`, formData)
      .then((res) => {
        setIsLoading(false)
        navigate(
          '/newArrival')
  ...
  const handleNewArrival = async () => {
    console.log(perfume, "perfume")
    const response = axios.post(`${import.meta.env.VITE_API_URL}/newArrival`, { newArrivalPerfume: perfume })
    ...
    <>
      <section className="bg-white ">
        <div className="max-w-4xl px-4 py-8 mx-auto lg:py-16">
          <h2 className="mb-4 text-xl font-bold text-gray-900 ">Add New Arrival Perfume</h2>
          <Toaster />
          <Select
          ...
          />
          {
            ...
            </div> : <div className="grid place-items-end py-4">
              <button
